...
from automation.constants import ARBITRUM_TOTAL
from automation.constants import ARBITRUM_TO_DISTRIBUTE
from automation.constants import BALANCER_GAUGE_CONTROLLER_ABI
from automation.constants import (
    BALANCER_GAUGE_CONTROLLER_ADDR,
)
from automation.constants import BALANCER_GRAPH_URL
from automation.constants import DYNAMIC_BOOST_MULTIPLIER
from automation.constants import GAUGES_WITH_BONUSES
from automation.constants import MIN_BAL_IN_USD_FOR_BOOST
from automation.constants import POOLS_SNAPSHOTS_QUERY
from automation.constants import VOTE_CAP_IN_PERCENT
...
from automation.static_boosts import boost_data
from automation.static_boosts import cap_override_data
...
def generate_and_save_transaction(arb_gauge_distributions: Dict, start_date: datetime, end_date: datetime) -> Dict:
    ...
    with open(
            f"{get_root_dir()}/output/dao_grant_{start_date.date()}_{end_date.date()}.json", "w"
    ) as _f:
...
def run_stip_pipeline(end_date: int) -> None:
    ...
    arb_gauge_distributions_df.to_csv(
        f"{get_root_dir()}/output/dao_grant_{start_date.date()}_{end_date.date()}_until_8march.csv", index=False
    )
